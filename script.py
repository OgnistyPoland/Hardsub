    ...
    for file in files:
        if file.endswith(".mkv"):
            ...
            if audio_channels == 2 and audio_codec != 'eac3':
                command = f'ffmpeg -i "processing.mkv" -vf "format=yuv420p,subtitles=processing.mkv" -map_metadata -1 -movflags faststart -c:v libx264 -profile:v main -level:v 4.1 -preset slow -crf 16 -x264-params colormatrix=bt709 -c:a copy "{abs_output_path}"'
            elif audio_channels == 6 or audio_codec == 'eac3':
                command = f'ffmpeg -i "processing.mkv" -vf "format=yuv420p,subtitles=processing.mkv" -map_metadata -1 -movflags faststart -c:v libx264 -profile:v main -level:v 4.1 -preset slow -crf 16 -x264-params colormatrix=bt709 -c:a libfdk_aac -b:a 256k "{abs_output_path}"'
            else:
